...
function displayRecipes(recipes) {
  ...
  for (let i = 0; i < recipes.length; i++) {
    let card = `<div class="col-lg-4"><div class="card"><img src="../img/${recipes[i].image}" class="card-img-top" alt="${recipes[i].name}" /><div class="card-body"><h2>${recipes[i].name}</h2><p><strong>Description: </strong><span class="description">${recipes[i].description}</span><br /><strong>Cook Time: </strong><span id="cook_time">${recipes[i].cookTime}</span><br /><strong>Category: </strong><span class="category">${recipes[i].category}</span></p></div><div class="card-footer text-center"><a href="recipe_details.html?id=${recipes[i].id}" class="btn btn-primary">View Recipe</a></div></div></div>`;
